def add_time(start_time, duration, day = None):
    ...
    if calculated_time["hours"] >= 12:
        ...
        calculated_time["time_of_day"] = (
            "AM" if calculated_time["hours"] == 2 or calculated_time["time_of_day"] == "PM" else
            "PM"
        )
        ...
        if given_time_of_day == "AM" and calculated_time["time_of_day"] == "PM":
            ...
            if day != None:
                ...
                calculated_time["days_apart"] = (
                    ...
                    f'(next day)' if calculated_time["hours"] * 12 == 24 or 24 // calculated_time["hours"] <= 1 else
                    calculated_time["days_apart"]
                )
            elif day == None:
                print(round(duration_time_obj["hour"] / 24))
                calculated_time["days_apart"] = (
                    f'({round(duration_time_obj["hour"] / 24)} days later)' if round(duration_time_obj["hour"] / 24) > 1 else
                    f'(next day)' if calculated_time["hours"] * 12 == 24 or round(duration_time_obj["hour"] / 24) <= 1 else
                    calculated_time["days_apart"]
                )
        else: 
            if day != None:
                day_index = days.index(day.lower())
                calculated_time["day"] = (
                    days[day_index + (round(duration_time_obj["hour"] / 24) - 1)].capitalize() if day_index + (round(duration_time_obj["hour"] / 24) + 1) > len(days) else 
                    days[day_index + (round(duration_time_obj["hour"] / 24))].capitalize()
                )
            calculated_time["days_apart"] = (
                f'({round(duration_time_obj["hour"] / 24)} days later)' if round(duration_time_obj["hour"] / 24) > 1 else
